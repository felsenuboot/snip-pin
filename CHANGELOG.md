...
### Changed
- Right-click on a pin opens the menu (as in Snipaste); `Ctrl+C` and a double-click copy and close. `right = copy` in `[mouse]` restores the old behaviour. In the history picker right-click opens a menu (pin, copy, keep, delete) instead of copying.

### Added
- Overlay: hovering or grabbing an anchor makes it the active one (orange); the arrow keys and `W A S D` then move that corner or edge by 1 px (`Shift`: 10) instead of the whole selection.
- The selection is snip-pin's own overlay (`snip-select.py`, GTK 4 + gtk4-layer-shell) over the frozen frame: windows and elements highlight under the pointer, a drag or click selects, anchors resize and the inside moves the selection, arrows nudge it, `Tab` cycles the detection mode and `1` / `2` or the wheel walk parent and child elements, `Ctrl+A` takes the monitor or everything, `R` / `Shift+R` reuse previous areas, `F5` refreshes the frame, `W A S D` move the pointer by a pixel, a magnifier shows the pixel grid, coordinate and colour (`C` copies it, `Shift` toggles HEX / RGB) and a panel lists the keys. `slurp` stays as the fallback (`selector = slurp`, or automatically without gtk4-layer-shell). Settings `sel_hints`, `sel_magnify`, `sel_adjust` (#73, #92, #93, #94, #95, #96, #97; part of #75).
