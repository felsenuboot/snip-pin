...
| Adjust | drag an anchor or the inside of a selection; arrows move it by 1 px (`Shift` 10, `Ctrl` resizes). Hover or grab an anchor (it turns orange) and the arrows or `W A S D` move that corner or edge instead. `Q` toggles adjust mode, in which a drag waits for `Enter` instead of capturing at once (`sel_adjust = 1` starts in it) |
